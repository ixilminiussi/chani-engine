...
InputComponent::InputComponent(Actor *ownerP)
    : MoveComponent(ownerP), maxForwardSpeed(100.0f), maxAngularSpeed(1.0f),
      forwardKey(SDL_SCANCODE_W), backKey(SDL_SCANCODE_S),
      clockwiseKey(SDL_SCANCODE_D), counterClockwiseKey(SDL_SCANCODE_A) {}
...
void InputComponent::processInput(const InputState &inputState) {
    ...
    if (inputState.keyboard.getKeyState(counterClockwiseKey) ==
        ButtonState::Held) {
        angularSpeed += maxAngularSpeed;
    }
...
void InputComponent::setForwardKey(SDL_Scancode key) { forwardKey = key; }

void InputComponent::setBackKey(SDL_Scancode key) { backKey = key; }

void InputComponent::setClockwiseKey(SDL_Scancode key) { clockwiseKey = key; }
