...
bool Texture::loadSDL(RendererSDL& renderer, const string& filenameP)
{
        ...
        if (!surf)
        {
                Log::error(LogCategory::Application, "Failed to load texture
file " + filename); return false;
        ...
        SDLTexture = SDL_CreateTextureFromSurface(renderer.toSDLRenderer(),
surf); SDL_DestroySurface(surf); if (!SDLTexture)
        {
                Log::error(LogCategory::Render, "Failed to convert surface to
texture for "+ filename); return false;
        }
