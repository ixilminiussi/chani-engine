...
void SplineCameraComponent::update(float dt) {
    ...
    Matrix4 view =
        Matrix4::createLookAt(cameraPosition, target, Vector3::unitZ);
    setViewMatrix(view);
}

void SplineCameraComponent::setSpeed(float speedP) { speed = speedP; }

void SplineCameraComponent::setSpline(const Spline &splineP) {
    spline = splineP;
}

void SplineCameraComponent::setPaused(bool isPausedP) { isPaused = isPausedP; }
