...
OrbitActor::OrbitActor()
    : Actor(), orbitCameraComponent(nullptr), targetActor(nullptr),
...
void OrbitActor::actorInput(const InputState &inputState) {
    Vector2 relativeMousePosition =
        inputState.mouse.getPosition() - prevMousePosition;
    ...
    orbitCameraComponent->zoom((float)inputState.mouse.getScrollWheel().y *
                               100.0f);

    prevMousePosition += relativeMousePosition;
}

void OrbitActor::snapToActor(Actor *actor) { targetActor = actor; }

void OrbitActor::updateActor(float dt) {
    if (targetActor != nullptr) {
        setPosition(Vector3<float>::lerp(getPosition(),
                                         targetActor->getPosition(), 0.2f));
    }
}
